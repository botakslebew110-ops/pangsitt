            ...
            <div style="max-width: 800px; margin: 0 auto; background-color: white; padding: 40px; border-radius: 10px; box-shadow: var(--shadow);">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 30px; margin-bottom: 40px;">
                    ...
                    <div style="text-align: center;">
                        ...
                        <p style="color: var(--gray);">Selesaikan pembayaran di aplikasi kami</p>
                    </div>
                </div>
                ...
                <form id="checkoutForm">
                    ...
                    <div class="checkout-form" id="step3Form" style="display: none;">
                        ...
                        <div style="background-color: var(--light-gray); border-radius: 8px; padding: 20px; margin-bottom: 30px;">
                            ...
                            <div id="finalOrderDetails">
                                <!-- Detail pesanan akan diisi oleh JavaScript -->
                            </div>
                        </div>
                        
                        <div style="text-align: center;">
                            <p style="color: var(--gray); margin-bottom: 20px;">Untuk menyelesaikan pembayaran, silakan buka aplikasi Pangsit Goreng atau klik tombol di bawah:</p>
                            <button type="button" class="btn" id="openAppButton" style="margin-bottom: 15px;">
                                <i class="fas fa-external-link-alt"></i> Buka di Aplikasi
                            </button>
                            <p style="font-size: 14px; color: var(--gray);">Atau scan QR code di bawah:</p>
                            <div style="background-color: white; padding: 15px; display: inline-block; border-radius: 8px; margin-top: 10px;">
                                <div style="width: 150px; height: 150px; background-color: #f0f0f0; display: flex; align-items: center; justify-content: center; color: var(--gray);">
                                    [QR Code Pembayaran]
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function createOrderFromCheckout() {
            try {
                ...
                const paymentMethod = selectedPayment ? selectedPayment.getAttribute('data-method') : 'transfer';
        ...
        function init() {
            ...
            document.getElementById('nextToStep2').addEventListener('click', () => {
                ...
                // Reset pilihan metode pembayaran
            ...
            document.getElementById('nextToStep3').addEventListener('click', () => {
                ...
                finalOrderDetails.innerHTML = `
                    ...
                    <div style="margin-bottom: 10px;">
                        <strong>Metode Pembayaran:</strong> ${paymentMethod.toUpperCase()}
                    </div>
                    <div style="margin-top: 15px;">
                        <strong>Items:</strong>
                        ${checkoutItems.map(item => `<div>${item.name} x${item.quantity} - ${formatRupiah(item.price * item.quantity)}</div>`).join('')}
                    </div>
                `;
            });
            ...
            // Event listener untuk tombol buka aplikasi
            document.getElementById('openAppButton').addEventListener('click', () => {
                // Simulasi membuka aplikasi
                showNotification('Membuka aplikasi Pangsit Goreng...');
                
                // Untuk simulasi, kita akan buka halaman baru
                setTimeout(() => {
                    window.open('https://play.google.com/store/apps/details?id=com.example.pangsitgoreng', '_blank');
                }, 1000);
            });
            ...
            console.log('✅ Sistem PANGS!T siap dengan fitur admin real-time');
        }
